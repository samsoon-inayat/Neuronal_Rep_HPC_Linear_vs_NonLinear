...
%% untuned or non-responsive cells (just testing one subset RMA)
pop_names = {'R','NR'};
all_cellsnew = []; all_cellsnew_vals = [];
for ii = 1:length(pop_names)
    cmdTxt = sprintf('all_cellsnew = [all_cellsnew propsTDM.newMI.cells_%s];',pop_names{ii}); eval(cmdTxt);
...
fac_names = {'PoT','CN','AP'}; fac_levels = [2,3,2];
...
% (Intercept) [F(1,4) = 353.20, p < 0.001, η2 = .96] <--
% (Intercept):AP [F(1,4) = 9.54, p = .037, η2 = .35] <--
% (Intercept):PoT:CN [F(2,8) = 7.04, p = .044, η2 = .10] <--


% (Intercept) [F(1,4) = 353.20, p < 0.001, η2 = .96] <--
% (Intercept):AP [F(1,4) = 9.54, p = .037, η2 = .34] <--
% (Intercept):PoT:CN [F(2,8) = 5.84, p = .037, η2 = .14] <--
...
avar = exec_fun_on_cell_mat(all_cellsnew,'percent');
fac_names = {'Mod','MT','Conf','AP'}; fac_levels = [2,2,3,2];
[within,dvn,xlabels,awithinD] = make_within_table(fac_names,fac_levels);
dataT = make_between_table({avar},dvn);
ra = RMA(dataT,within,{0.05,{''}});
clc
...
%%
clc
raR = RMA_bonferroni(ra,'Mod');

%%
clc
raRR1 = RMA_bonferroni(raR{2},'Mod');
%%
clc
...
[hbs,xdata,mVar,semVar,combs,p,h] = view_results_rmanova([],raR{2},{'Mod:Conf','hsd',0.05},[1 1.75],tcolors,[mY MY ysp ystf ysigf],mData);
...
avar = exec_fun_on_cell_mat(all_cellsnew,'percent');
fac_names = {'Mod','MT','Conf','AP'}; fac_levels = [length(pop_names),2,3,2];
[within,dvn,xlabels,awithinD] = make_within_table(fac_names,fac_levels);
dataT = make_between_table({avar},dvn);
ra = RMA(dataT,within,{0.05,{''}});
clc
...
MY = 100; ysp = 10; mY = 0; ystf = 10; ysigf = 0.05;titletxt = ''; ylabeltxt = {'PDF'}; % for all cells (vals) MY = 80
[hbs,xdata,mVar,semVar,combs,p,h] = view_results_rmanova(ff.h_axes(1,1),raR{2},{'AP:Mod','hsd',0.05},[1 1.5],tcolors,[mY MY ysp ystf ysigf],mData);
% make_bars_hollow(hbs(2))
format_axes(gca);
set(gca,'xcolor','k','ycolor','k','xlim',xlim,'ylim',ylim,...
    'XTick',xdata,'XTickLabel',{'Sing','Mix','All'});xtickangle(20);
ylabel({'Cells (%)'});
set_bar_graph_sub_xtick_text(ff.hf,gca,hbs,3,{'Air-On','Air-Off'},{[0 0]});
ht = set_axes_top_text_no_line(ff.hf,gca,sprintf('MI'),[0.051 0.0 0 0]); 
...
for ii = 1:length(pop_names)
    ...
    % all_cellsnew_peakvalsT = [all_cellsnew_peakvalsT propsPL.peak_locations_trialsM propsPL.peak_locations_trialsM];
...
tcolors = repmat(mData.colors(1:3),1,2);
% figure(300);clf; ha = gca;
ff = makeFigureRowsCols(2020,[5 4 1.25 1],'RowsCols',[1 1],'spaceRowsCols',[0.07 0],'rightUpShifts',[0.25 0.335],'widthHeightAdjustment',[-300 -380]);
MY = 45; ysp = 5; mY = 0; ystf = 5; ysigf = 0.05;titletxt = ''; ylabeltxt = {'PDF'}; % for all cells (vals) MY = 80
