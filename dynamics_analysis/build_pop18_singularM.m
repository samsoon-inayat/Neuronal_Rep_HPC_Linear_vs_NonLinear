function [pop, labels18] = build_pop18_singular(idv, case_labels)
...
A = size(idv,1); C = size(idv,2); assert(C==6,'Expect 6 cases');
labs3 = {'T','D','S'};
...
M = C*3; X = cell(A,M); labels18 = cell(1,M);

m = 0;
for t = 1:C
    for b = 1:3
        m = m + 1;
        labels18{m} = sprintf('%s-%sM', case_labels{t}, labs3{b});
        for a = 1:A
